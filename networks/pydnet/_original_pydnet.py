...
class Encoder(nn.Module):
    def __init__(self, params):
        ...
        channel_encoder_features    = [16,32,64,96,128,192]
        self.num_ch_enc = np.array(channel_encoder_features)
        self.conv1 = self.encoder_block(3, channel_encoder_features[0])
        self.conv2 = self.encoder_block(channel_encoder_features[0], channel_encoder_features[1])
        self.conv3 = self.encoder_block(channel_encoder_features[1], channel_encoder_features[2])
        self.conv4 = self.encoder_block(channel_encoder_features[2], channel_encoder_features[3])
        self.conv5 = self.encoder_block(channel_encoder_features[3], channel_encoder_features[4])
        self.conv6 = self.encoder_block(channel_encoder_features[4], channel_encoder_features[5])
    ...
    def encoder_block(self, channel_in, channel_out):
        ...
        '''
        layers = []
        layers.append(conv2d_leaky(channel_in=channel_in,  channel_out=channel_out, kernel_size=3, stride=2, relu=True))
        layers.append(conv2d_leaky(channel_in=channel_out, channel_out=channel_out, kernel_size=3, stride=1, relu=True))
...
class Decoder(nn.Module):
    def __init__(self, params):
        ...
        num_ch = 8
        self.upsample_features_6 = bilinear_upsampling_by_deconvolution(in_ch=num_ch, out_ch=num_ch, kernel=2)
        self.upsample_features_5 = bilinear_upsampling_by_deconvolution(in_ch=num_ch, out_ch=num_ch, kernel=2)
        self.upsample_features_4 = bilinear_upsampling_by_deconvolution(in_ch=num_ch, out_ch=num_ch, kernel=2)
        self.upsample_features_3 = bilinear_upsampling_by_deconvolution(in_ch=num_ch, out_ch=num_ch, kernel=2)
        self.upsample_features_2 = bilinear_upsampling_by_deconvolution(in_ch=num_ch, out_ch=num_ch, kernel=2)

        self.estimator_6 = self.estimator(channel_encoder_features[5])            #H/64
        self.estimator_5 = self.estimator(channel_encoder_features[4] + num_ch)   #H/32
    ...
    def forward(self, encoder_features):
        ...
        disp_3 = self.get_disparity(estimator_3)
        ...
        disp_2 = self.get_disparity(estimator_2)

        features_1 = torch.cat((encoder_features_1, upsampled_features_2), 1)
        estimator_1 = self.estimator_1(features_1)
        disp_1 = self.get_disparity(estimator_1)

        predictions = [None, disp_1, disp_2, disp_3]
        self.outputs = {}
    ...
    def estimator(self, channel_in):
        ...
        '''
        layers = []
        layers.append(conv2d_leaky(channel_in=channel_in,  channel_out=96,  kernel_size=3, relu=True))
        layers.append(conv2d_leaky(channel_in=96,  channel_out=64,  kernel_size=3, relu=True))
        layers.append(conv2d_leaky(channel_in=64,  channel_out=32,  kernel_size=3, relu=True))
        layers.append(conv2d_leaky(channel_in=32,  channel_out=8,   kernel_size=3, relu=False))
        return nn.Sequential(*layers)
